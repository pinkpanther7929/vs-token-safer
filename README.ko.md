...
| 레이어 | 파일 | 효과 |
| --- | --- | --- |
| **재작성/강제 훅(hook)** | `hooks/block-code-grep.js` | 소스 파일(`.c/.cc/.cpp/.h/.hpp/.cs/.ts/.py`…, 또는 `src/`·`source/`·`engine/`·`plugins/`)을 겨냥한 Bash `grep`/`rg`/`ack`/`ag`/`findstr`/`git grep`과 `find -name`을 가로챕니다. 명령이 안전한 단일 세그먼트라면 그 자리에서 동등한 `vts` 쿼리로 바꿔 줍니다(식별자는 의미 기반 `search_symbol`로, 리터럴은 `search_text`로, `find -name`은 `find_files`로). 모델의 작업 흐름이 끊기지 않고 출력은 언제나 토큰 캡이 걸립니다. 명령이 모호하면(파이프라인이나 비리터럴 패턴) 하드 차단으로 물러납니다. 발동 조건은 까다롭습니다. 세그먼트가 실제로 실행하는 파일이 검색 도구일 때만 끼어들고, 원시 텍스트 검색(로그·`.md`·`.json`·설정·build 디렉터리)은 그대로 통과시킵니다. `VTS_REWRITE=0`이면 재작성 대신 차단하고, `excludeCommands`로 특정 명령을 빼며, `VTS_ENFORCE=0`이면 아예 끕니다. |
...
- 목-LSP eval(`node eval/run.mjs`, 툴체인 불필요)이 매 커밋 응답 정형화 절감을 게이트합니다: raw 인덱스
  `~57,308 tok` → 캡된 출력 `~1,549 tok` = **97.3%** (체크 41/41).
...
- **훅**은 Bash 호출이 일어나기 전마다 실행됩니다. 명령이 코드 검색(grep/rg/ack/ag/findstr/`git grep`,
  또는 코드 확장자 `*.c/.cc/.cpp/.h/.hpp/.cs/.ts/.tsx/.js/.jsx/.mjs/.cjs/.py`·`src|source|engine|plugins/`를
  겨냥한 `find -name`)인데 로그·md·json·빌드 경로는 *아닐* 때, 그 자리에서 동등한 `vts` 쿼리로 바꿔 줍니다
  (식별자는 의미 기반 `symbol`로, 리터럴은 `text`로, `find -name`은 `files`로). 덕분에 검색은 토큰 캡이 걸린
  채 그대로 돌아갑니다. 안전한 재작성을 만들 수 없으면(파이프라인이나 셸 메타문자 패턴) 차단으로 물러납니다.
  `VTS_REWRITE=0`이면 차단만 하고, `excludeCommands`나 `VTS_EXCLUDE_COMMANDS`로 제외하며, `VTS_ENFORCE=0`이면 완전히 끕니다.
